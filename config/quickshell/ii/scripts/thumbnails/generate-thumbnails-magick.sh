#!/usr/bin/env bash
# Generate thumbnails for files using ImageMagick and ffmpegthumbnailer, following Freedesktop spec
# Usage:
...
generate_video_thumbnail() {
    ...
    # Use ffmpegthumbnailer for video files
    # -s: thumbnail size, -t: time in video (10% through), -q: quality (0-10)
    ffmpegthumbnailer -i "$abs_path" -o "$out" -s "$THUMBNAIL_SIZE" -t 10% -q 8 2>/dev/null || return
}
...
generate_image_thumbnail() {
    ...
    # For GIFs, take first frame only
    if [[ "${abs_path,,}" == *.gif ]]; then
...
generate_thumbnail() {
    ...
    # Detect file type and use appropriate thumbnail generator
    case "${abs_path,,}" in
        *.mp4|*.webm|*.mkv|*.avi|*.mov|*.flv|*.m4v|*.wmv)
            generate_video_thumbnail "$src"
            ;;
