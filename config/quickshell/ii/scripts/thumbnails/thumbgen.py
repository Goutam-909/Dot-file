...
def make_thumbnail(args) -> bool:
    fpath, size = args
    # Initialize factory in each worker process
...
@logger.catch()
def thumbnail_folder(*, dir_path: Path, workers: int, only_images: bool, recursive: bool, machine_progress: bool = False, size: str = "normal") -> None:
    all_files = get_all_files(dir_path=dir_path, recursive=recursive)
    if only_images:
        all_files = get_all_images(all_files=all_files)
    all_files_with_size = [(str(fpath), size) for fpath in all_files]
    if machine_progress:
        completed = 0
        total = len(all_files_with_size)
        with Pool(processes=workers) as p:
            for result in p.imap(make_thumbnail, all_files_with_size):
                completed += 1
                print(f"PROGRESS {completed}/{total}")
                sys.stdout.flush()
    else:
        with Pool(processes=workers) as p:
            list(tqdm(p.imap(make_thumbnail, all_files_with_size), total=len(all_files_with_size)))
...
@click.command()
@click.option("-d", "--img_dirs", required=True, help='directories to generate thumbnails')
@click.option("-s", "--size", default="normal", type=click.Choice(["normal", "large", "x-large", "xx-large"]))
@click.option("-w", "--workers", default=1, help="no of cpus to use")
@click.option("-i", "--only_images", is_flag=True, default=False)
@click.option("-r", "--recursive", is_flag=True, default=False)
@click.option("--machine_progress", is_flag=True, default=False)
def main(img_dirs: str, size: str, workers: int, only_images: bool, recursive: bool, machine_progress: bool) -> None:
    img_dirs = [Path(img_dir) for img_dir in img_dirs.split()]
    for img_dir in img_dirs:
        thumbnail_folder(dir_path=img_dir, workers=workers, only_images=only_images, recursive=recursive, machine_progress=machine_progress, size=size)
