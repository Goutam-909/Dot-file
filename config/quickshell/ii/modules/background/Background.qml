...
Variants {
    id: root
    readonly property bool fixedClockPosition: Config.options.background.clock.fixedPosition ?? false
    readonly property real fixedClockX: Config.options.background.clock.x ?? 0
    readonly property real fixedClockY: Config.options.background.clock.y ?? 0
    ...
    PanelWindow {
        ...
        property var activeWorkspaceWithFullscreen: workspacesForMonitor.filter(workspace=>{
            if (!workspace) return false;
            const fullscreenWindows = workspace.toplevels.values.filter(window => window && window.wayland && window.wayland.fullscreen);
            return (fullscreenWindows.length > 0 && workspace.active);
        })[0]
