...
Item {
    ...
    PopupWindow {
        ...
        MouseArea {
            ...
            hoverEnabled: true
            x: {
                // Safety check before calling mapFromItem
                if (root.QsWindow && root.lastHoveredButton &&
                    root.lastHoveredButton.Window.window) {
                    const itemCenter = root.QsWindow.mapFromItem(
                        root.lastHoveredButton,
                        root.lastHoveredButton.width / 2, 0
                    );
                return itemCenter.x - width / 2;
                    }
                    return 0; // Safe default when mapping not possible
            }
