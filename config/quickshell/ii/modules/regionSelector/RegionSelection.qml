    ...
    Process {
        ...
        command: ["bash", "-c", `${Directories.scriptPath}/images/find-regions-venv.sh ` 
            ...
            stdout: StdioCollector {
                ...
                onStreamFinished: {
                    try {
                        if (imageDimensionCollector.text && imageDimensionCollector.text.trim().length > 0) {
                            imageRegions = filterImageRegions(
                                JSON.parse(imageDimensionCollector.text),
                                                              root.windowRegions
                            );
                        } else {
                            console.warn("[Region Selector] No image regions detected");
                            imageRegions = [];
                        }
                    } catch (e) {
                        console.error("[Region Selector] Failed to parse image regions:", e);
                        console.error("Raw output was:", imageDimensionCollector.text);
                        imageRegions = [];
                    }
                }
            }
