...
Singleton {
    ...
    function apply(path, darkMode = Appearance.m3colors.darkmode) {
        if (!path || path.length === 0) return
            ...
            console.log(`[Wallpapers] Applying wallpaper: ${cleanPath} (darkMode: ${darkMode})`)

            // Call switchwall.sh with proper arguments
            // Use setsid to detach the process so it doesn't block Quickshell
            applyProc.exec([
                "setsid",
                Directories.wallpaperSwitchScriptPath,
                "--image", cleanPath,
            ...
            // Update the config with the new wallpaper path
            Config.options.background.wallpaperPath = cleanPath
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            // It's a file, apply it as wallpaper
    ...
    function randomFromCurrentFolder(darkMode = Appearance.m3colors.darkmode) {
        ...
        console.log("[Wallpapers] Randomly selected wallpaper:", filePath);
